        ...
        with self._generation_lock():
            ...
            if self.enable_expense_analysis and self._is_expense_query(user_message_body):
                ...
                try:
                    ...
                    # Don't return here, fall through to other processing
            
            elif self.enable_inventory_analysis and self._is_inventory_query(user_message_body):
                _logger.info("PROCESSING INVENTORY QUERY")
                try:    
                    ...
                    _logger.info("INVENTORY ANALYSIS COMPLETED")
                    yield {"type": "message_create", "message": assistant_message.message_format()[0]}
            ...
            # ✅ FIXED: Moved outside expense analysis block
            try:
            ...
            try:
                ...
                _logger.error(f"Generate_messages failed: {e}", exc_info=True)
                # Create error response
                error_message = self.message_post(
                    body="Sorry, I encountered an error while generating a response. Please try again.",
                    llm_role="assistant",
                    author_id=False
                )
